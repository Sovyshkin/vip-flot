...
```
Внешний вид → Темы → 
  Деактивировать "Галерея яхт" (выберите любую другую тему)
  Активировать "Галерея яхт" снова
```
...
# Затем:
cd /Users/vadim/Desktop/gallery-yachts/app
npm run serve
```
...
Откройте:
```
http://localhost/gallery-yachts/wp-admin/admin-ajax.php?action=gallery_yachts_booking
```
...
```bash
curl -X POST 'http://localhost/gallery-yachts/wp-admin/admin-ajax.php' \
  -H 'Origin: http://localhost:8080' \
  -d 'action=gallery_yachts_booking' \
...
1. **`gallery-yachts-ajax.php`** (создан)
...
2. **`functions.php`** (обновлен)
   - Добавлено подключение gallery-yachts-ajax.php

### Файлы Vue:

1. **`.env`** (обновлен)
   ```
   VUE_APP_API_URL=http://localhost/gallery-yachts/wp-admin/admin-ajax.php
   ```

2. **`RequestBook.vue`** (обновлен)
   - Используется FormData вместо JSON
   - Добавлен параметр `action=gallery_yachts_booking`
...
3. **Проверьте файлы созданы**
   ```
   gallery-yachts-theme/gallery-yachts-ajax.php - должен существовать
   gallery-yachts-theme/gallery-yachts-cors.php - должен существовать
   ```
...
1. Обновите `.env` с реальным URL:
   ...
   ```
2. В `gallery-yachts-ajax.php` добавьте домен фронта в `$allowed_origins`
